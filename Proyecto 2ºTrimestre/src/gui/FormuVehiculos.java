...
/**
 * Formulario 'gen�rico' para todos los veh�culos
 * @author Victor
 *
 */
public class FormuVehiculos extends JDialog {
	...
	private FormuVehiculos() {
		...
		//Formamos los paneles que vamos a utilizar y pedimos los atributos generales que no cab�an arriba
	...
	private void declaraAtributosVentana() {
		...
		Dimension dim = Toolkit.getDefaultToolkit().getScreenSize();	//Averiguamos el tama�o de la pantalla
	...
	private void creaFormuCocheElectrico(JTabbedPane EspecialidadVehiculos) {
		...
		JLabel lblcategoriaCocheElectrico = new JLabel("Categoría");
		...
		}
		{
			...
			tbMatricula.addKeyListener(new KeyAdapter() {
				...
				public void keyPressed(KeyEvent e) {
					...
					{
						//Rellenamos los campos seg�n la base de datos
...
//				getRootPane().setDefaultButton(okButton);
	...
	private void rellenaVehiculoElegido() {
		...
		try {
			...
			JOptionPane.showMessageDialog(cbEmisiones, "Matrícula no válida: "+tbMatricula.getText(),
		...
		{//Si es eléctrico
			...
			{//Si es un CocheElectrico
				...
				//Si es electrico pero no es un CocheElectrico, será una moto
				VehiculoElegido = RepositorioVehiculo.leeMotocicleta(matricula);
			}
		}else {
			//Si no es eléctrico, será de combustión
			if (RepositorioVehiculo.CocheCombustion(mat)) {
				try {
					...
					JOptionPane.showMessageDialog(cbEmisiones, "Error al crear el coche de combustión: "+tbMatricula.getText(),
							"Error de creación",JOptionPane.ERROR_MESSAGE);
					e.printStackTrace();
				}
			}else {
				//Sólo puede ser una furgoneta
	...
	private void rellenaVehiculoFromBD(Matricula mat) {
		// Primero realizamos la consulta seg�n su matricula
		try {
				...
				try {
					...
					default:
						JOptionPane.showMessageDialog(cbTipo,
													  "Tipo de veh�culo no reconocido",
